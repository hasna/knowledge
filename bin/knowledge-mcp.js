...
var HASNA_KNOWLEDGE_APP_PATH, LEGACY_HASNA_KNOWLEDGE_APP_PATH, CANONICAL_KNOWLEDGE_SCOPES, trustedWorkspaceIdentities;
...
function runtimeFor(options) {
  ...
  }, () => readKnowledgeConfiguredMode(resolveScopedWorkspace(options.scope ?? "project", options.cwd).configPath));
}
...
async function startMcpHttpServer(buildServer, options = {}) {
  ...
  const initialRuntime = runtimeFor(options);
  ...
  const httpServer = createServer(async (req, res) => {
    try {
      ...
      if (req.method === "GET" && url.pathname === "/ready") {
        const current2 = runtimeFor(options);
      ...
      const current = runtimeFor(options);
...
Options:
  ...
  if (wantsHttp(argv, env)) {
    ...
    const { buildServer } = await import(payloadPath2);
