...
describe('knowledge cli', () => {
  test('help and subcommand help work', () => {
    ...
    const contextOut = new TextDecoder().decode(context.stdout);
    expect(contextOut).toContain('knowledge context pack');
    expect(contextOut).toContain('--allow-global');

    const proposals = runCli(['help', 'proposals']);
    expect(proposals.exitCode).toBe(0);
    const proposalsOut = new TextDecoder().decode(proposals.stdout);
    expect(proposalsOut).toContain('knowledge proposals context');
    expect(proposalsOut).toContain('--allow-global');
  });
  ...
  test('global notes added through CLI are searchable and available as context', () => {
    ...
    const inventory = runCli(['inventory', '--scope', 'global', '--allow-global', '--json'], undefined, env);
    expect(inventory.exitCode).toBe(0);
    ...
    expect(deniedPack.exitCode).toBe(1);
    expect(new TextDecoder().decode(deniedPack.stderr)).toContain('allowGlobal=true');
