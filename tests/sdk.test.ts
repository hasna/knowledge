...
describe('public knowledge sdk', () => {
  test('exposes a stable client facade for installed apps', async () => {
    ...
    expect(readFileSync(source, 'utf8')).toContain('SDK facade');
    expect(() => readFileSync(join(dir, '.hasna', 'knowledge', 'config.json'), 'utf8')).toThrow();

    expect(client.storage.writeBoundary({ strict: true }).ok).toBe(false);
    expect(client.projectPanel({ projectId: 'sdk-project' }).metadata.knowledge_db_exists).toBe(false);
