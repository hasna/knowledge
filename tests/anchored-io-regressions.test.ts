...
import * as anchoredFsTest from '../src/anchored-fs';
import {
...
describe('directory-anchored config and artifact regressions', () => {
  ...
  test('same-key put serialization gives the outer writer bounded deterministic forward progress', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'knowledge-artifact-same-key-forward-progress-'));
    const root = join(fixture, 'artifacts');
    const target = join(root, 'same.txt');
    const artifacts = new AnchoredArtifactDirectory(root);
    const setLockControl = (anchoredFsTest as unknown as {
      setAnchoredArtifactLockTestControl?: (control: unknown) => void;
    }).setAnchoredArtifactLockTestControl;
    let monotonic = 0;
    let injected = false;
    let nestedError = '';
    artifacts.put('same.txt', 'baseline');
    setLockControl?.({
      monotonicNow: () => monotonic,
      wait: (milliseconds: number) => { monotonic += milliseconds; },
    });
    setAnchoredFsTestHook((event, detail) => {
      if (event !== 'artifact-before-final-verify' || detail !== 'same.txt' || injected) return;
      injected = true;
      try {
        artifacts.put('same.txt', 'nested-candidate');
      } catch (error) {
        nestedError = error instanceof Error ? error.message : String(error);
      }
    });
    try {
      expect(() => artifacts.put('same.txt', 'outer-candidate')).not.toThrow();
      expect(injected).toBe(true);
      expect(nestedError).toMatch(/lock|progress|acquire/i);
      expect(readFileSync(target, 'utf8')).toBe('outer-candidate');
      expect(readdirSync(root).filter((name) => name.startsWith('.knowledge-'))).toEqual([]);
    } finally {
      setAnchoredFsTestHook(undefined);
      setLockControl?.(undefined);
      rmSync(fixture, { recursive: true, force: true });
    }
  });

  test('canonical artifact-key aliases serialize as one target with bounded progress', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'knowledge-artifact-alias-key-progress-'));
    const root = join(fixture, 'artifacts');
    const target = join(root, 'nested', 'same.txt');
    const outerKey = 'nested\\same.txt';
    const innerKey = 'nested/same.txt';
    const artifacts = new AnchoredArtifactDirectory(root);
    const setLockControl = (anchoredFsTest as unknown as {
      setAnchoredArtifactLockTestControl?: (control: unknown) => void;
    }).setAnchoredArtifactLockTestControl;
    ...
    let injected = false;
    let nestedError = '';
    artifacts.put(innerKey, 'baseline');
    setLockControl?.({
      monotonicNow: () => monotonic,
      wait: (milliseconds: number) => { monotonic += milliseconds; },
    });
    setAnchoredFsTestHook((event, detail) => {
      if (event !== 'artifact-before-final-verify' || detail !== outerKey || injected) return;
      injected = true;
      try {
        artifacts.put(innerKey, 'nested-candidate');
      } catch (error) {
        nestedError = error instanceof Error ? error.message : String(error);
      }
    });
    try {
      expect(() => artifacts.put(outerKey, 'outer-candidate')).not.toThrow();
      expect(injected).toBe(true);
      expect(nestedError).toMatch(/lock|progress|acquire/i);
      expect(readFileSync(target, 'utf8')).toBe('outer-candidate');
      expect(readdirSync(root).filter((name) => name.startsWith('.knowledge-'))).toEqual([]);
  ...
  test('contention retries never accept unsafe static artifact lock records', () => {
    for (const mutation of ['symlink', 'mode', 'malformed'] as const) {
      const fixture = mkdtempSync(join(tmpdir(), `knowledge-artifact-lock-${mutation}-`));
      const root = join(fixture, 'artifacts');
      const outside = join(fixture, 'outside.lock');
      const key = 'guarded.txt';
      const lockName = `.knowledge-artifact-lock-${createHash('sha256').update(key).digest('hex')}`;
      const lockPath = join(root, lockName);
      const artifacts = new AnchoredArtifactDirectory(root);
      writeFileSync(outside, 'outside-unchanged', { mode: 0o600 });
      if (mutation === 'symlink') symlinkSync(outside, lockPath);
      else if (mutation === 'mode') {
        writeFileSync(lockPath, `${JSON.stringify({ version: 1, pid: process.pid, token: 'static-owner' })}\n`, { mode: 0o600 });
        chmodSync(lockPath, 0o640);
      } else {
        writeFileSync(lockPath, 'malformed-static-lock\n', { mode: 0o600 });
      }
      try {
        expect(() => artifacts.put(key, 'must-not-install')).toThrow(/artifact key lock/i);
