...
function printCommandHelp(command: string): void {
  ...
  if (command === 'upsert') { console.log(`Usage: knowledge upsert [title] [content] [--id <id>] [--title <title>] [--content <content>] [--description <text>] [--url <url>] [-t <tag>]... [--json]\n  --description (${DESCRIPTION_MIN_LENGTH}-${DESCRIPTION_MAX_LENGTH} chars) is REQUIRED when this command CREATES an item, and optional when it\n  UPDATES one — an ordinary edit never has to restate it. That asymmetry is why it is not simply\n  listed as required: upserting an existing id without it succeeds and leaves the stored value alone,\n  while upserting an id that does not exist yet is a create and is refused without it.\n  -t/--tag is repeatable and accepts comma-separated values; tags are added, never replaced.\n  With -t the output reports how many tags were actually added, on both the create and update paths.`); return; }
