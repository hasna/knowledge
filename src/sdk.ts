...
export function createKnowledgeClient(options: KnowledgeClientOptions = {}): KnowledgeClient {
  ...
  return {
    ...
    wiki: {
      ...
      fileAnswer: (input) => service.fileAnswer(withReadAuthority(input)),
    ...
    ask: (prompt, input = {}) => {
      ...
      return service.runPrompt({ ...withReadAuthority(input), prompt });
    },
    build: (prompt, input = {}) => {
      assertPublicInvocation([], { surface: 'sdk' });
      service.modelRegistry();
...
function withDefaultAllowGlobal<T extends { allowGlobal?: boolean } | undefined>(
  ...
  let descriptor: PropertyDescriptor | undefined;
  if (allowGlobal === true && input && typeof input === 'object') {
    try {
      descriptor = Object.getOwnPropertyDescriptor(input, 'allowGlobal');
    } catch {
      throw new Error('Global wiki access requires an explicit own allowGlobal data property.');
    }
