...
function stableResolverEvidence(value: Record<string, unknown>): Record<string, unknown> {
...
function cacheabilityFromUnknown(value: unknown): KnowledgeMachineRouteResolution['cacheability'] {
...
function cacheabilityIsFresh(cacheability: KnowledgeMachineRouteResolution['cacheability'], now: string): boolean {
  if (!cacheability || cacheability.stale) return false;
  if (!cacheability.expires_at) return true;
...
function routeMatchesResolverEvidence(route: KnowledgeMachineRouteResolution, previous: Record<string, unknown>): boolean {
  return stringFromUnknown(previous.source) === route.source
    && stringFromUnknown(previous.target) === route.target
    && stringFromUnknown(previous.route) === route.route
    && stringFromUnknown(previous.target_kind) === route.targetKind
    && stringFromUnknown(previous.confidence) === route.confidence;
}

function workspaceMatchesResolverEvidence(workspace: KnowledgeMachineWorkspaceResolution, previous: Record<string, unknown>): boolean {
  return stringFromUnknown(previous.source) === workspace.source
    && stringFromUnknown(previous.requested_machine_id) === workspace.requested_machine_id
    && stringFromUnknown(previous.machine_id) === workspace.machine_id
    && stringFromUnknown(previous.project_id) === workspace.project_id
    && stringFromUnknown(previous.repo_name) === workspace.repo_name
    && stringFromUnknown(previous.project_root) === workspace.project_root
    && stringFromUnknown(previous.project_root_source) === workspace.project_root_source
    && stringFromUnknown(previous.workspace_root) === workspace.workspace_root
    && stringFromUnknown(previous.workspace_root_source) === workspace.workspace_root_source
    && stringFromUnknown(previous.open_files_root) === workspace.open_files_root
    && stringFromUnknown(previous.open_files_root_source) === workspace.open_files_root_source
    && stringFromUnknown(previous.trust_status) === workspace.trust_status
    && stringFromUnknown(previous.auth_status) === workspace.auth_status
    && booleanFromUnknown(previous.current) === workspace.current
    && booleanFromUnknown(previous.primary) === workspace.primary;
}

function routeWithStableCacheability(
  route: KnowledgeMachineRouteResolution | null,
  previous: Record<string, unknown>,
  now: string,
): KnowledgeMachineRouteResolution | null {
...
function workspaceWithStableCacheability(
  workspace: KnowledgeMachineWorkspaceResolution | null,
  previous: Record<string, unknown>,
  now: string,
): KnowledgeMachineWorkspaceResolution | null {
...
export function recordKnowledgeMachineResolverEvidence(dbPath: string, input: KnowledgeMachineResolverEvidenceInput): KnowledgeSyncMachineRow {
  ...
  try {
    ...
    const route = routeWithStableCacheability(
      input.route?.source === 'registry' ? null : input.route ?? null,
      previousRouteEvidence,
      now,
    );
    const workspace = workspaceWithStableCacheability(
      input.workspace?.source === 'registry' ? null : input.workspace ?? null,
      previousWorkspaceEvidence,
      now,
    );
    ...
    if (existing) {
      ...
      const unchanged = existing.workspace_home === (workspace?.project_root ?? existing.workspace_home ?? null)
        && existing.tailscale_dns === resolverTailscaleDns(existing, route)
        && existing.ssh_target === (route?.target ?? existing.ssh_target ?? null)
        && canonicalJson(parseJsonRecord(existing.capabilities_json)) === canonicalJson(nextCapabilities)
        && canonicalJson(stableResolverEvidence(existingResolverEvidence)) === canonicalJson(stableResolverEvidence(nextResolverEvidence));
...
async function materializeArtifacts(options: {
  ...
  for (const artifact of options.bundle.artifacts) {
    ...
    if (target && artifactFingerprint(target) !== artifactFingerprint(artifact)) {
      const conflict: KnowledgeSyncConflictInput = {
...
function hasResolvedConflictFingerprint(db: Database, input: KnowledgeSyncConflictInput): boolean {
  const localHash = input.localHash ?? '';
  const remoteHash = input.remoteHash ?? '';
  const exact = db.query<{ id: string }, [
    string,
    string,
    string,
    string,
  ...
  const reversed = db.query<{ id: string }, [
    string,
    string,
    string,
    string,
  ...
  try {
    ...
    for (const sourceTable of options.bundle.tables) {
      ...
      for (const sourceRow of sourceTable.rows) {
        ...
        result.conflicts += 1;
        if (!dryRun && insertConflict(db, conflict)) conflictsCreated += 1;
      }
