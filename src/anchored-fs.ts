...
function artifactLockStatIsSafeTransition(stat: Stats, links: number): boolean {
  const transitionalLinkCount = links === 1
...
function recoverArtifactLockTransition(
  ...
  if (finalStat) {
    ...
    if (!final) {
      ...
      if (!refreshed || refreshed.nlink === 1) return 'retry';
  ...
  try {
    ...
    for (const segment of segments) {
      ...
      try {
        ...
        if (next !== undefined) closeSync(next);
  ...
  try {
    ...
    if (fd !== undefined) {
      try {
        ...
        // Preserve the original failure; close the descriptor even when the
        // temporary name was concurrently removed or replaced.
      }
      closeSync(fd);
    }
  }
...
function recoverRegularGeneration(
  ...
  if (sameIdentity(identity(current), identity(previous))) {
    if (
      ...
      || (current.nlink !== 2 && current.nlink !== 3)
    ) {
    ...
    if (!recovered) fail(`${kind} canonical generation disappeared during recovery`);
    assertRecoverableGenerationStat(
      recovered,
      expectedMode,
      maxBytes,
      [expectedLinks],
      `${kind} recovered canonical generation`,
    );
  } else {
