...
function createMcpRuntimeContext(options) {
  ...
  const serviceFor = (requestedScope, defaultScope = 'global') => {
    ...
    const slot = slots.get(canonicalScope);
    if (!slot) throw new Error(`Missing startup-bound MCP workspace for ${canonicalScope}.`);
...
export function buildServer(options = {}) {
  const runtimeContext = createMcpRuntimeContext(options);
