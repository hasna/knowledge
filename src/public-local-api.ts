...
function guardedGlobalRead<T extends (options: any, ...rest: any[]) => any>(implementation: T): T {
  const wrapped = guarded(((options: Parameters<T>[0], ...rest: unknown[]) => {
...
export const buildKnowledgeAgentContextPack = guarded(buildKnowledgeAgentContextPackImpl);
...
export const runKnowledgePrompt = guarded(runKnowledgePromptImpl);
